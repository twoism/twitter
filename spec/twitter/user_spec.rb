...
describe Twitter::User do
  ...
  describe "#==" do
    it "returns true when ids and classes are equal" do
      user = Twitter::User.new(:body => {'id' => 1})
    ...
    it "returns false when ids are not equal" do
      user = Twitter::User.new(:body => {'id' => 1})
      other = Twitter::User.new(:body => {'id' => 2})
      (user == other).should be_false
    end
  end
  ...
  describe "#created_at" do
    it "returns a Time when created_at is set" do
      user = Twitter::User.new(:body => {'created_at' => "Mon Jul 16 12:59:01 +0000 2007"})
  ...
  describe "#status" do
    it "returns a Status when status is set" do
      status = Twitter::User.new(:body => {'status' => {}}).status
    ...
    it "includes a User when user is set" do
      status = Twitter::User.new(:body => {'screen_name' => 'sferik', 'status' => {}}).status
