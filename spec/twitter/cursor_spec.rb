...
describe Twitter::Cursor do
  ...
  describe "#first?" do
    context "when previous cursor equals zero" do
      before do
        @cursor = Twitter::Cursor.from_response({:body => {'previous_cursor' => 0}}, 'ids')
      end
      it "returns true" do
    ...
    context "when previous cursor does not equal zero" do
      before do
        @cursor = Twitter::Cursor.from_response({:body => {'previous_cursor' => 1}}, 'ids')
      end
      it "returns true" do
  ...
  describe "#last?" do
    context "when next cursor equals zero" do
      before do
        @cursor = Twitter::Cursor.from_response({:body => {'next_cursor' => 0}}, 'ids')
      end
      it "returns true" do
    ...
    context "when next cursor does not equal zero" do
      before do
        @cursor = Twitter::Cursor.from_response({:body => {'next_cursor' => 1}}, 'ids')
      end
      it "returns false" do
