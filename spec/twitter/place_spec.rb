...
describe Twitter::Place do
  ...
  describe "#==" do
    it "returns true when ids and classes are equal" do
      place = Twitter::Place.new(:body => {'id' => 1})
      other = Twitter::Place.new(:body => {'id' => 1})
    ...
    it "returns false when ids are not equal" do
      place = Twitter::Place.new(:body => {'id' => 1})
      other = Twitter::Place.new(:body => {'id' => 2})
      (place == other).should be_false
    end
  end
  ...
  describe "#bounding_box" do
    it "returns a Twitter::Place when set" do
      place = Twitter::Place.new(:body => {'bounding_box' => {'type' => 'Polygon', 'coordinates' => [[[-122.40348192, 37.77752898], [-122.387436, 37.77752898], [-122.387436, 37.79448597], [-122.40348192, 37.79448597]]]}})
  ...
  describe "#country_code" do
    it "returns a country code when set with country_code" do
      place = Twitter::Place.new(:body => {'country_code' => 'US'})
      place.country_code.should eq 'US'
    end
    it "returns a country code when set with countryCode" do
      place = Twitter::Place.new(:body => {'countryCode' => 'US'})
      place.country_code.should eq 'US'
    end
    it "returns nil when not set" do
      place = Twitter::Place.new
  ...
  describe "#parent_id" do
    it "returns a parent ID when set with parentid" do
      place = Twitter::Place.new(:body => {'parentid' => 1})
  ...
  describe "#place_type" do
    it "returns a place type when set with place_type" do
      place = Twitter::Place.new(:body => {'place_type' => 'city'})
      place.place_type.should eq 'city'
    end
    it "returns a place type when set with placeType[name]" do
      place = Twitter::Place.new(:body => {'placeType' => {'name' => 'Town'}})
