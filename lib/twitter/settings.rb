...
module Twitter
  class Settings < Twitter::Base
    ...
    def trend_location
      @trend_location ||= Twitter::Place.from_response(:body => @attrs['trend_location'].first, :response_headers => self.response_headers) unless @attrs['trend_location'].nil? || @attrs['trend_location'].empty?
    end
